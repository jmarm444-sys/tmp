#!/usr/bin/env python3
"""
Stock History Manager — symbol summary view (variant C)
=======================================================

Variant of stkhist_gui_A.py with a master–detail layout plus sorting and a
bulk refresh:
  ...
  * Add / update / delete of individual records works on the bottom
    ...
    is unchanged from stkhist_gui_A.py.
  * The summary table can be sorted by symbol (A-Z / Z-A) or by the latest
    daily % change (highest / lowest), via the toolbar combobox or by
    clicking the "Symbol" / "% Change" column headings.
  * "Refresh All (Today)" button re-downloads the current day's values for
    every symbol stored in the database.

Run with:  python3 stkhist_gui_C.py
"""
...
# Sort options for the summary table; each maps to a SQL ORDER BY clause.
# The % change used is the one from each symbol's latest stored date.
SORT_ORDERS = {
    "Symbol (A-Z)": "symbol ASC",
    "Symbol (Z-A)": "symbol DESC",
    "% Change (Highest)": "pct_change IS NULL, pct_change DESC, symbol ASC",
    "% Change (Lowest)": "pct_change IS NULL, pct_change ASC, symbol ASC",
}
DEFAULT_SORT = "Symbol (A-Z)"
...
class StockDB:
    ...
    def fetch_summary(self, symbol=None, sort=DEFAULT_SORT):
        ...
        if symbol:
            sql += " WHERE symbol = ?"
            params = (symbol,)
        sql += f" GROUP BY symbol ORDER BY {order_by}"
...
class App(tk.Tk):
    ...
    def __init__(self):
        super().__init__()
        self.title("Stock History Manager (summary view C)  —  stkhist.db")
    ...
    def _build_menu(self):
        ...
        edit_menu.add_command(
            label="Refresh ALL symbols (today)", command=self.refresh_all_symbols
        )
    ...
    def _build_toolbar(self):
        ...
        ttk.Button(
            bar, text="Refresh All (Today)", command=self.refresh_all_symbols
        ).pack(side="left", padx=4)

        ttk.Label(bar, text="Sort by:").pack(side="left", padx=(16, 4))
        ...
        sort_box.bind("<<ComboboxSelected>>", lambda _e: self.refresh_table())
    ...
    def _toggle_sort(self, primary, secondary):
        """Heading click: apply *primary* sort, or *secondary* if already active."""
        self.sort_var.set(secondary if self.sort_var.get() == primary else primary)
        self.refresh_table()
    ...
    def refresh_table(self):
        ...
        rows = self.db.fetch_summary(filter_symbol, sort=sort)
        for row in rows:
    ...
    def fetch_symbols(self, symbols, period=None):
        period = period or self.period_var.get()
        errors, total_rows = [], 0
    ...
    def refresh_all_symbols(self):
        """Re-download the current day's values for every stored symbol."""
        symbols = self.db.symbols()
        if not symbols:
            messagebox.showinfo(
                "No symbols", "The database contains no symbols to refresh."
            )
            return
        if not messagebox.askyesno(
            "Refresh all symbols",
            f"Fetch today's values for all {len(symbols)} symbol(s)?",
        ):
            return
        # period="1d" fetches only the current trading day's row.
        self.fetch_symbols(symbols, period="1d")
